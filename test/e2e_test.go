...
func testACRPullBinding[B binding](
	...
	t.Run("credential provisioning failures", func(t *testing.T) {
		...
		t.Logf("updating pull binding %s/%s to refer to missing service account", namespace, pullBinding)
		if err := client.Update(ctx, createBinding(namespace, pullBinding, "repository:alice:pull", "missing", cfg)); err != nil {
		...
		if err := client.Update(ctx, createBinding(namespace, pullBinding, "repository:alice:pull", serviceAccount, cfg)); err != nil {
			t.Fatalf("failed to update pull binding %s/%s: %v", namespace, pullBinding, err)
		}
		eventuallyFulfillPullBinding[B](t, ctx, client, namespace, pullBinding, newBinding)
